function [Hb_SDCcor, ScansQuality] = preprocessing_PMI_localizer_online(raw, qtflag, NFB_Loc)
...
for sub = 1:length(dHbx_div)
    ...
    nirs.design.save_stim2excel(dHbx_div(sub),'stim.xlsx');

    %Split the runs
    [split_data]=split_runs_localizer_online('stim.xlsx', no_trials, no_runs, dHbx_div(sub), NFB_Loc(sub));
...
% % % % % Hb_normalized = job.run(Hb_normalized);
